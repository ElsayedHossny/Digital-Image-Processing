...
pi_grey = cv2.imread("Kewy.png",flags = 0) #we used grey photo for simpler calculations(flag = 0)

c = 255 / np.log(1+np.max(pi_grey))
img_grey_logarithmic = c * (np.log(1+pi_grey))
img_grey_logarithmic = np.array(img_grey_logarithmic,dtype = np.uint8) #the logarithmic generate floating point values
...
cv2.imshow("pi grey logarithmic", img_grey_logarithmic)
cv2.waitKey()
...
import cv2
import numpy as np
pi_grey = cv2.imread("orange.png",flags = 1) #we used grey photo for simpler calculations(flag = 0)
C = 1
gamma = 1.5 #random chosen value
gamma_corrected = np.array(255 * C * (pi_grey/255) ** gamma, dtype = np.uint8)
...
pi_cv2 = cv2.imread(r"orange.png", flags = 1)
...
pi_original = cv2.imread("pi_grey.JPEG", flags = 1)
...
figure_all_in_one = plt.figure(figsize=(6,8)) #إنشاء الشكل الذي يحتوي على 4 صور (صورة أصلية و3 صور تحويلات غاما مختلفة).
rows = 2 #contain 2 rows and 2 columns
columns = 2
...
for gamma in gamma_values:
    ...
    plt.subplot(rows, columns, i) #plt.subplot(rows, columns, index)
    i = i + 1
