...
kernel = np.array([[0, -1, 0],
                   [-1, 4, -1],
                   [0,-1, 0]], dtype=np.float32) # Kernel Laplacian

filtered_image = cv2.filter2D(src=image , ddepth=-1 , kernel=kernel) #ddepth=-1 تعني أن ناتج الفلترة سيأخذ نفس نوع البيانات الخاصة بالصورة الأصلية
...
gaussian_kernel_1d = cv2.getGaussianKernel(5, 3) # filter (5x1) 1d

gaussian_kernel_2d = gaussian_kernel_1d @ gaussian_kernel_1d.T # convert from (1D) to (2D)

blurred_image = cv2.filter2D(image, -1, gaussian_kernel_2d) #تطبيق الفلتر علي الصوره ,-1 same information from orig img
